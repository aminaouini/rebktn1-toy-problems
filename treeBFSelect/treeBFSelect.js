...
Tree.prototype.BFSelect = function(filter) {
  ...
  for(var i = 0; i < childs.length; i++) {
    if ( !filter(childs[i].value, childs[i]) ) result.push(childs[i].value);

    for (let j = 0; j < array.length; j++) {
      if( !filter(childs[i].children[j].value, childs[i].children[j] ) ) result.push(childs[i].children[j].value)
